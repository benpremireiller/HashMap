...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        entry_at_index = self._buckets[hash_index]

        entry = HashEntry(key, value)

        # Find the next empty spot using quadratic probing
        index_populated = entry_at_index is not None and not entry_at_index.is_tombstone
        next_index = hash_index
        j = 1
        
        while index_populated and entry_at_index.key != key:
            next_index = (hash_index + j ** 2) % self._capacity
            entry_at_index = self._buckets[next_index]
            index_populated = entry_at_index is not None and not entry_at_index.is_tombstone
            j += 1

        # Increment size if the entry is not replacing one with same key
        if entry_at_index is None or entry_at_index.is_tombstone:
            self._size += 1

        self._buckets[next_index] = entry
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        # Copy over old values to new array with new hashes
    ...
    def get(self, key: str) -> object:
        ...
        j = 1
        while entry_at_index is not None:
            if entry_at_index.key == key and not entry_at_index.is_tombstone:
                return entry_at_index.value
    ...
    def remove(self, key: str) -> None:
        ...
        while entry_at_index is not None:
            if entry_at_index.key == key and not entry_at_index.is_tombstone:
                entry_at_index.is_tombstone = True
                self._size -= 1
                return
    ...
    def clear(self) -> None:
        """
        Clear this hashmap by removing all it's values
        """
    ...
    def __iter__(self):
        """
        Return a reference to this map. Used for iteration.
        """
    ...
    def __next__(self):
        """
        Return the value at the next index. Used for iteration.
        """
